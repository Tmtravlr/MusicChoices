...
public class MusicEventHandler
{
	...
	public void onMusic(PlayBackgroundMusicEvent event)
	{
		...
		boolean isArtificalLight = chunk.getSavedLightValue(EnumSkyBlock.Block, x & 15, y, z & 15) >= 7;
		...
		String lightingType = isSky ? (isDay ? "Day" : isArtificalLight ? "Ambient" : "Night") : (isArtificalLight ? "Ambient" : "Cave");

		if (MusicChoicesMod.DEBUG)
		{
			System.out.println("[Music Choices] The music track \'" + event.result.getSoundUrl() + "\' is attempting to play.");
		}
		...
		try
		{
			if (MusicChoicesMod.DEBUG)
			{
				System.out.println("[Music Choices] Searching through music folders for music tracks to play.");
			}
			...
			if (BossStatus.bossName != null && BossStatus.statusBarLength > 0)
			{
				...
				File bossFolder = new File(folder, "Battle");
				...
				directories.add(bossFolder);
				directories.add(new File(bossFolder, BossStatus.bossName));
				bossPresent = true;
			}
			...
			if (!bossPresent || !MusicChoicesMod.ONLY_BOSS_MUSIC_FOR_BOSSES)
			{
				...
				lightingFolder = new File(musicFolder, "Lighting");
				dimensionLightingFolder = new File(folder, "Lighting");
				
				if (!lightingFolder.exists())
				{
					lightingFolder.mkdir();
				}
				
				if (!dimensionLightingFolder.exists())
				{
					dimensionLightingFolder.mkdir();
				}
				
				directories.add(new File(lightingFolder, lightingType));
				directories.add(new File(dimensionLightingFolder, lightingType));
				...
				File biomeFolder = new File(biomesFolder, biomeName);
				File dimensionBiomeFolder = new File(dimensionBiomesFolder, biomeName);
				directories.add(biomeFolder);
				directories.add(dimensionBiomeFolder);

				lightingFolder = new File(biomeFolder, "Lighting");
				dimensionLightingFolder = new File(dimensionBiomeFolder, "Lighting");
				...
				for (int i = 0; i < BiomeDictionary.getTypesForBiome(biome).length; ++i)
				{
					...
					File biomeTypeFolder = new File(biomeTypesFolder, biomeType.name());
					File dimensionBiomeTypeFolder = new File(dimensionBiomeTypesFolder, biomeType.name());
					
					directories.add(biomeTypeFolder);
					directories.add(dimensionBiomeTypeFolder);
					
					lightingFolder = new File(biomeTypeFolder, "Lighting");
					dimensionLightingFolder = new File(dimensionBiomeTypeFolder, "Lighting");
			...
			for(File directory : directories)
			{
				if (MusicChoicesMod.DEBUG)
				{
					System.out.println("[Music Choices]  * Looking in directory \'" + directory.getPath() + "\'");
				}
				...
				if (!directory.exists())
				{
					...
					if (MusicChoicesMod.DEBUG)
					{
						System.out.println("[Music Choices]    * \'" + directory.getPath() + "\' didn\'t exist yet. Attempting to create it. Successful? " + successful);
					}
				}
				...
				if (directory.isDirectory() && directory.listFiles().length > 0)
				{
					for (int i = 0; i < directory.listFiles().length; ++i)
					{
						...
						if (musicFile != null && musicFile.exists() && musicFile.getName().substring(musicFile.getName().lastIndexOf(46) + 1).equals("ogg"))
						{
							...
							if (MusicChoicesMod.DEBUG)
							{
								System.out.println("[Music Choices]    * Found a .ogg file! Adding the file \'" + musicFile.getName() + "\' to list of music to choose from.");
							}
						}
					}
				}
			...
			if (musicToChooseFrom.size() > 0)
			{
				...
				if (MusicChoicesMod.DEBUG)
				{
					System.out.println("[Music Choices] Found " + musicToChooseFrom.size() + " music file" + (musicToChooseFrom.size() == 1 ? "" : "s") + ". Changed music playing to \'" + musicChoice.getName() + "\'");
				}
			}
			else if (MusicChoicesMod.DEBUG)
			{
				System.out.println("[Music Choices] Didn\'t find any music to play in this circumstance. Playing default music.");
			}
		}
		catch (Exception e)
		{
			System.err.println("[Music Choices] Error while playing custom music! =(");
			e.printStackTrace();
		}
	}
	...
	public static SoundPoolEntry getMenuMusic()
	{
		if (MusicChoicesMod.DEBUG)
		{
			System.out.println("[Music Choices] Attempting to play a Menu music track.");
		}
		...
		try
		{
			if (MusicChoicesMod.DEBUG)
			{
				System.out.println("[Music Choices] Looking in the \'Menu\' folder for music to play.");
			}
			...
			if (musicFiles != null && musicFiles.length > 0)
			{
				...
				if (!musicToPlay.getName().substring(musicToPlay.getName().lastIndexOf(46) + 1).equals("ogg"))
				{
					if (MusicChoicesMod.DEBUG)
					{
						System.out.println("[Music Choices] The selected music track wasn\'t a .ogg file! Only .ogg files can play.");
					}
				...
				if (MusicChoicesMod.DEBUG)
				{
					System.out.println("[Music Choices] Playing Menu music track \'" + musicToPlay.getName() + "\'");
				}
			...
			if (MusicChoicesMod.DEBUG)
			{
				System.out.println("[Music Choices] Couldn\'t find any Menu music to play.");
			}
		}
		catch (Exception e)
		{
			System.err.println("[Music Choices] Error while playing custom menu music! =(");
			e.printStackTrace();
		}
	...
	public static SoundPoolEntry getBossMusic()
	{
		if (BossStatus.bossName != null && BossStatus.statusBarLength > 0)
		{
			try
			{
				if (MusicChoicesMod.DEBUG)
				{
					System.out.println("[Music Choices] Attempting to play immediate boss music.");
				}
				...
				File specificBossDir = new File(battleDir, BossStatus.bossName);
				...
				for (i = 0; i < battleDir.listFiles().length; ++i)
				{
					file = battleDir.listFiles()[i];
				...
				if (musicToChooseFrom.size() > 0)
				{
					...
					if (MusicChoicesMod.DEBUG)
					{
						System.out.println("[Music Choices] Found " + musicToChooseFrom.size() + " music file" + (musicToChooseFrom.size() == 1 ? "" : "s") + ". Playing boss music \'" + musicChoice.getName() + "\'");
					}
				...
				if (MusicChoicesMod.DEBUG)
				{
					System.out.println("[Music Choices] Didn\'t find any immediate boss music to play for this boss: \'" + BossStatus.bossName + "\'.");
				}
			}
			catch (Exception e)
			{
				System.err.println("[Music Choices] Error while playing custom boss music! =(");
				e.printStackTrace();
			}
	...
	public static SoundPoolEntry getVictoryMusic()
	{
		...
		else
		{
			try
			{
				if (MusicChoicesMod.DEBUG)
				{
					System.out.println("[Music Choices] Attempting to play boss victory music.");
				}
				...
				if (MusicChoicesMod.DEBUG)
				{
					System.out.println("[Music Choices] Didn\'t find any boss victory music to play for this boss: \'" + BossStatus.bossName + "\'.");
				}
			}
			catch (Exception e)
			{
				System.err.println("[Music Choices] Error while playing custom victory music! =(");
				e.printStackTrace();
			}
	...
	public static SoundPoolEntry getAchievementMusic(Achievement a)
	{
		try
		{
			if (MusicChoicesMod.DEBUG)
			{
				System.out.println("[Music Choices] Attempting to play achievement music.");
			}
			...
			if (musicToChooseFrom.size() > 0)
			{
				...
				if (MusicChoicesMod.DEBUG)
				{
					System.out.println("[Music Choices] Found " + musicToChooseFrom.size() + " music file" + (musicToChooseFrom.size() == 1 ? "" : "s") + ". Playing achievement music \'" + var10.getName() + "\'");
				}
			...
			if (MusicChoicesMod.DEBUG)
			{
				System.out.println("[Music Choices] Didn\'t find any achievement music to play for the achievement \'" + a.getName() + "\'.");
			}
		}
	...
	public void onCredits(GuiOpenEvent event)
	{
		if (event.gui instanceof GuiWinGame)
		{
			try
			{
				...
				for (int i = 0; i < toPlay; ++i)
				{
					...
					if (file != null && file.exists() && file.getName().substring(file.getName().lastIndexOf(46) + 1).equals("ogg"))
					{
						...
						if (MusicChoicesMod.DEBUG)
						{
							System.out.println("[Music Choices] Found a .ogg file! Adding the file \'" + file.getName() + "\' to list of music to choose from.");
						}
					}
				}
			...
			{
				System.err.println("[Music Choices] Error while playing credits music! =(");
				e.printStackTrace();
			}
		}
	}
